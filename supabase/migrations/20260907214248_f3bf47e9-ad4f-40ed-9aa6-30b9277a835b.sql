-- ============ F02: perfil obrigatório dentro das funções privilegiadas ============

CREATE OR REPLACE FUNCTION public.assert_office_or_admin(_action text)
RETURNS void
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  IF auth.uid() IS NULL THEN
    RAISE EXCEPTION 'Sessão inválida para %', _action;
  END IF;
  IF NOT (public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'escritorio')) THEN
    RAISE EXCEPTION 'Sem permissão para %', _action;
  END IF;
END;
$function$;

REVOKE ALL ON FUNCTION public.assert_office_or_admin(text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.assert_office_or_admin(text) TO authenticated, service_role;
...
BEGIN
  PERFORM public.assert_office_or_admin('cancelar encomendas');
...
DECLARE
  ...
  v_new numeric;
BEGIN
  PERFORM public.assert_office_or_admin('concluir produção para stock');
...
-- ============ F03: o código do operador tem de corresponder à sessão ============
...
AS $function$
BEGIN
  -- Sem sessão (contexto de serviço) ou operador sem conta ligada: nada a validar aqui.
  IF auth.uid() IS NULL OR _op.user_id IS NULL THEN
    RETURN;
  END IF;
...
REVOKE ALL ON FUNCTION public.assert_operator_is_session(public.operators) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.assert_operator_is_session(public.operators) TO authenticated, service_role;
...
BEGIN
  ...
  IF _event='iniciar' AND v_cs.status='em_curso' AND NOT v_cs.is_paused THEN
    RAISE EXCEPTION 'Coli já está em curso nesta etapa';
  END IF;
...
-- Regra que devia limitar operadores estava permissiva (somava acesso).
...
-- Escritas diretas em volumes/etapas de volumes: só admin/escritório.
-- O trabalho dos postos passa pelas funções registadas (SECURITY DEFINER).
DROP POLICY IF EXISTS "auth write order_colis" ON public.order_colis;
CREATE POLICY "office write order_colis" ON public.order_colis
...
DROP POLICY IF EXISTS "auth write order_coli_stages" ON public.order_coli_stages;
CREATE POLICY "office write order_coli_stages" ON public.order_coli_stages
  FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'escritorio'))
  WITH CHECK (public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'escritorio'));

DROP POLICY IF EXISTS "operator/escritorio write stages" ON public.order_stages;
CREATE POLICY "office write stages" ON public.order_stages
  FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'escritorio'))
  WITH CHECK (public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'escritorio'));