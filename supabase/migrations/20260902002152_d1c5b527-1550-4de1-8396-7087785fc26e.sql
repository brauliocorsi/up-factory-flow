...
DECLARE
  ...
  v_operators jsonb;
BEGIN
  -- minutos produtivos das etapas concluídas hoje
  ...
  -- tempo já acumulado em etapas ainda em curso hoje
  SELECT COALESCE(SUM(
           os.productive_seconds
           + CASE WHEN NOT os.is_paused AND os.last_resume_at IS NOT NULL
                  THEN GREATEST(0, EXTRACT(EPOCH FROM (v_now - os.last_resume_at))::int)
                  ELSE 0 END
         ), 0)
    INTO v_running_sec
  FROM order_stages os
  ...
  -- encomendas com data de saída hoje
  SELECT COUNT(*),
  ...
  -- SLA das etapas concluídas hoje
  ...
  -- minutos produtivos por bloco horário do turno
  ...
  -- operadores em tempo real (apenas nome e encomenda)
  ...
  FROM (
    SELECT DISTINCT ON (os.operator_id)
      jsonb_build_object(
        ...
        'last_resume_at', os.last_resume_at,
