...
| 02 | Autorização servidor + SQL, leituras dos postos | Não iniciada | `production.functions.ts`, `stock.functions.ts`, `roleGuards.ts`, auth middleware, políticas | prevista | — | — |
| 03 | Coerência etapas OP/volume, rotas, reparação | Não iniciada | `create_order_colis`, `sync_order_stage_from_colis`, `colis.functions.ts` | prevista | — | 12 rotas sem etapas |
