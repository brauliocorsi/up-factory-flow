...
| 13 | Tempos por pessoa e ajustes de interface | **Pendente** | `analytics.functions.ts`, `producao.index.tsx` | não prevista | — | repartição de mão de obra por participante e por dia (Europe/Lisbon) exige histórico por intervalos; `order_coli_stages` não guarda registo por intervalo como `stage_time_logs`, pelo que a correção honesta implica novo histórico de eventos por volume |
