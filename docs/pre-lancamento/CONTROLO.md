...
| 14 | Fecho de permissões, integridade e regressão integrada | **Concluída (parcial declarada)** | inventário global de funções e escritas, `docs/pre-lancamento/ENTREGA.md`, `TESTES-MANUAIS.md` | não | inventário da base: todas as funções internas (`assert_*`, `create_order_colis`, `sync_order_stage_from_colis`, `try_reserve_for_order`, `*_impl`, gatilhos, `get_public_factory_panel`) sem execução para `anon`/`authenticated` — só a base as usa; inventário do código: todas as escritas em encomendas/etapas passam por funções de servidor com verificação de perfil (admin/escritório) ou por funções da base com autorização própria; tipos 0 erros; 18 rotas verificadas (`/`, `/producao`, `/encomendas`, `/encomendas/importar-simples`, `/picagem`, `/picagem/historico`, `/picagem/consulta`, `/retrabalho`, `/painel`, `/stock/tecidos`, `/stock/cascos`, `/stock/capas`, `/admin/qualidade`, `/admin/relatorios`, `/admin/planeamento`, `/admin/rotas-colis`, `/admin/sla`, `/configuracoes`) → 200 | regressão integrada dos 14 percursos com pessoas reais, leitor de códigos, impressora e recetor de stock continua por fazer no terreno (`TESTES-MANUAIS.md`); 44 avisos `SECURITY DEFINER` mantidos por decisão (as funções autorizam internamente); 5 operadores ativos sem conta ligada (03, 04, 07, 09, 10) |
